...
import org.apache.tinkerpop4.gremlin.structure.Graph.OptOut
...
import org.apache.tinkerpop4.gremlin.process.traversal.strategy.decoration.TranslationStrategy
import org.apache.tinkerpop4.gremlin.jsr223.JavaTranslator
import org.apache.tinkerpop4.gremlin.tinkercat.structure.io.graphson.AbstractTinkerCatGraphSONTranslatorProvider
...
import org.apache.tinkerpop4.gremlin.tinkercat.structure.io.graphson.AbstractTinkerCatGraphSONTranslatorProvider.TinkerCatGraphSONv2TranslatorProvider
import org.apache.tinkerpop4.gremlin.tinkercat.structure.io.graphson.AbstractTinkerCatGraphSONTranslatorProvider.TinkerCatGraphSONv3TranslatorProvider
 ...
 */
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.map.ProfileTest",
    method = "*",
    reason = "Tests for profile() are not supported for remotes"
)
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.branch.BranchTest",
...
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.branch.ChooseTest",
    method = "g_V_chooseXlabel_eqXpersonX__outXknowsX__inXcreatedXX_name",
    reason = "Tests that include lambdas are not supported by the test suite for remotes"
)
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.filter.DedupTest",
    method = "g_V_both_name_order_byXa_bX_dedup_value",
    reason = "Tests that include lambdas are not supported by the test suite for remotes"
)
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.filter.FilterTest",
...
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.sideEffect.GroupTest",
...
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.filter.HasTest",
    method = "g_V_outXcreatedX_hasXname__mapXlengthX_isXgtX3XXX_name",
    reason = "Tests that include lambdas are not supported by the test suite for remotes"
)
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.sideEffect.InjectTest",
    method = "g_VX1X_out_name_injectXdanielX_asXaX_mapXlengthX_path",
    reason = "Tests that include lambdas are not supported by the test suite for remotes"
)
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.map.MapTest",
...
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.map.OrderTest",
...
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.branch.RepeatTest",
    method = "g_V_repeatXbothX_untilXname_eq_marko_or_loops_gt_1X_groupCount_byXnameX",
    reason = "Tests that include lambdas are not supported by the test suite for remotes"
)
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.sideEffect.SackTest",
    method = "g_withSackXmap__map_cloneX_V_out_out_sackXmap_a_nameX_sack",
    reason = "Tests that include lambdas are not supported by the test suite for remotes"
)
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.sideEffect.SideEffectTest",
...
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.sideEffect.SubgraphTest",
...
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.map.UnfoldTest",
    method = "g_V_valueMap_unfold_mapXkeyX",
    reason = "Tests that include lambdas are not supported by the test suite for remotes"
)
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.map.ProgramTest",
    method = "*",
    reason = "Reason requires investigation"
)
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.TraversalInterruptionTest",
    method = "*",
    reason = "Reason requires investigation"
)
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.TraversalInterruptionComputerTest",
    method = "*",
    reason = "Reason requires investigation"
)
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.strategy.decoration.EventStrategyProcessTest",
    method = "*",
    reason = "Strategy not properly supported by Bytecode based traversals"
)
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.strategy.decoration.ElementIdStrategyProcessTest",
    method = "*",
    reason = "Strategy not properly supported by Bytecode based traversals"
)
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.CoreTraversalTest",
    method = "shouldNeverPropagateANoBulkTraverser",
    reason = "Reason requires investigation"
)
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.map.ReadTest",
    method = "*",
    reason = "read and write tests don't translate locally well because of calling iterate() inside read()/write() add a none()"
)
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.map.WriteTest",
    method = "*",
    reason = "read and write tests don't translate locally well because of calling iterate() inside read()/write() add a none()"
)
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.sideEffect.StoreTest",
...
@OptOut(
    test = "org.apache.tinkerpop4.gremlin.process.traversal.step.sideEffect.AggregateTest",
    method = "g_withSideEffectXa_setX_V_both_name_aggregateXlocal_aX_capXaX",
    reason = "This test returns BulkSet which isn't supported in GraphSON 3.0 until 3.4.0."
)
abstract class AbstractTinkerCatGraphSONTranslatorProvider internal constructor(private val version: GraphSONVersion) :
    TinkerCatProvider() {
    override fun traversal(graph: Graph): GraphTraversalSource {
        val g = graph.traversal()
        return g.withStrategies(TranslationStrategy(g, GraphSONTranslator(JavaTranslator.of(g), version), true))
    }

    open class TinkerCatGraphSONv2TranslatorProvider : AbstractTinkerCatGraphSONTranslatorProvider(GraphSONVersion.V2_0)
    open class TinkerCatGraphSONv3TranslatorProvider : AbstractTinkerCatGraphSONTranslatorProvider(GraphSONVersion.V3_0)

    @GraphProvider.Descriptor(computer = TinkerCatComputer::class)
    @OptOut(
        test = "org.apache.tinkerpop4.gremlin.process.computer.GraphComputerTest",
        method = "shouldSucceedWithProperTraverserRequirements",
        reason = "Reason requires investigation"
    )
    class TinkerCatGraphSONv2TranslatorComputerProvider : TinkerCatGraphSONv2TranslatorProvider() {
        override fun traversal(graph: Graph): GraphTraversalSource {
    ...
    class TinkerCatGraphSONv3TranslatorComputerProvider : TinkerCatGraphSONv3TranslatorProvider() {
        override fun traversal(graph: Graph): GraphTraversalSource {
            return super.traversal(graph).withComputer()
        }
    }
