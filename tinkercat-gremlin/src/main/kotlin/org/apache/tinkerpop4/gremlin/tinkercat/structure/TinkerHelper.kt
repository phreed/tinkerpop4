...
import org.apache.tinkerpop4.gremlin.process.computer.VertexComputeKey
import org.apache.tinkerpop4.gremlin.structure.*
import org.apache.tinkerpop4.gremlin.structure.util.ElementHelper
import org.apache.tinkerpop4.gremlin.tinkercat.process.computer.TinkerCatComputerView
import java.util.*
...
object TinkerHelper {
    ...
    fun addEdge(
        ...
        edge = idValue?.let { TinkerEdge(it, outVertex, label, inVertex) }!!
    ...
    private fun addOutEdge(vertex: TinkerVertex, label: String, edge: Edge?) {
        if (null == vertex.outEdges) vertex.outEdges = mutableMapOf()
    ...
    private fun addInEdge(vertex: TinkerVertex, label: String, edge: Edge?) {
        if (null == vertex.inEdges) vertex.inEdges = mutableMapOf()
    ...
    fun getEdges(vertex: TinkerVertex, direction: Direction, vararg edgeLabels: String?): Iterator<TinkerEdge> {
        ...
        if ((direction == Direction.OUT || direction == Direction.BOTH) && vertex.outEdges != null) {
            if (edgeLabels.isEmpty()) vertex.outEdges!!.values.forEach(Consumer { c: Set<Edge>? ->
                edges.addAll(c!!)
            ...
            ) else Stream.of(*edgeLabels).map { key -> vertex.outEdges!![key] }
                .filter { obj -> Objects.nonNull(obj) }
                .forEach { c ->
                    edges.addAll( c!! )
                }
        }
        if ((direction == Direction.IN || direction == Direction.BOTH) && vertex.inEdges != null) {
            if (edgeLabels.isEmpty()) vertex.inEdges!!.values.forEach(Consumer { c: Set<Edge>? ->
                edges.addAll(c!!)
            }) else if (edgeLabels.size == 1) edges.addAll(
            ...
            ) else Stream.of(*edgeLabels).map { key -> vertex.inEdges!![key] }
                .filter { obj -> Objects.nonNull(obj) }
                .forEach { c ->
                    edges.addAll( c!! )
                }
        }
        return edges.iterator() as Iterator<TinkerEdge>
    }
    ...
    fun getVertices(vertex: TinkerVertex, direction: Direction, vararg edgeLabels: String?): Iterator<TinkerVertex> {
        ...
        if ((direction == Direction.OUT || direction == Direction.BOTH) && vertex.outEdges != null) {
            if (edgeLabels.isEmpty()) vertex.outEdges!!.values.forEach(Consumer { set: Set<Edge> ->
                set.forEach(Consumer { edge: Edge -> vertices.add((edge as TinkerEdge).inVertex) })
            }) else if (edgeLabels.size == 1) vertex.outEdges!!.getOrDefault(edgeLabels[0],
                emptySet()).forEach(Consumer { edge: Edge -> vertices.add((edge as TinkerEdge).inVertex) })
            else Stream.of(*edgeLabels)
        ...
        if ((direction == Direction.IN || direction == Direction.BOTH) && vertex.inEdges != null) {
            if (edgeLabels.isEmpty()) vertex.inEdges!!.values.forEach(Consumer { set: Set<Edge> ->
                set.forEach(Consumer { edge: Edge -> vertices.add((edge as TinkerEdge).outVertex) })
        ...
        return vertices.iterator() as Iterator<TinkerVertex>
    }
