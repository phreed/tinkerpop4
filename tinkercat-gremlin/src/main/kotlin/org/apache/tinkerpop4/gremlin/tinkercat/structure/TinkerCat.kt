...
import org.apache.tinkerpop4.gremlin.structure.Graph.Features.*
...
import org.apache.tinkerpop4.gremlin.tinkercat.process.traversal.strategy.optimization.TinkerCatCountStrategy
...
//    }
    ...
    private fun saveGraph() {
        ...
        try {
            when (graphFormat) {
                "graphml" -> io(IoCore.graphml()).writeGraph(graphLocation)
                "graphson" -> io(IoCore.graphson()).writeGraph(graphLocation)
                "gryo" -> io(IoCore.gryo()).writeGraph(graphLocation)
                else -> io(IoCore.createIoBuilder(graphFormat)).writeGraph(graphLocation)
            }
        } catch (ex: Exception) {
