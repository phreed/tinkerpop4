...
public final class TinkerIoRegistryV3d0 extends AbstractIoRegistry {
     ...
     * list).
     */
    final static class TinkerCatJacksonSerializer extends StdScalarSerializer<TinkerCat> {
    ...
    static class TinkerCatJacksonDeserializer extends StdDeserializer<TinkerCat> {
        ...
        @Override
        public TinkerCat deserialize(final JsonParser jsonParser, final DeserializationContext deserializationContext) throws IOException, JsonProcessingException {
            final Configuration conf = new BaseConfiguration();
            conf.setProperty("gremlin.tinkercat.defaultVertexPropertyCardinality", "list");
            final TinkerCat graph = TinkerCat.open(conf);
