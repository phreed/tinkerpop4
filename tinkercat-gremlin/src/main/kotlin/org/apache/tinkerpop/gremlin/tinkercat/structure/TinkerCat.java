...
public final class TinkerCat implements Graph {

    static {
        TraversalStrategies.GlobalCache.registerStrategies(TinkerCat.class, TraversalStrategies.GlobalCache.getStrategies(Graph.class).clone().addStrategies(
                TinkerCatStepStrategy.instance(),
                TinkerCatCountStrategy.instance()));
    }

    private static final Configuration EMPTY_CONFIGURATION = new BaseConfiguration() {{
        this.setProperty(Graph.GRAPH, TinkerCat.class.getName());
    }};

    public static final String GREMLIN_TINKERGRAPH_VERTEX_ID_MANAGER = "gremlin.tinkercat.vertexIdManager";
    public static final String GREMLIN_TINKERGRAPH_EDGE_ID_MANAGER = "gremlin.tinkercat.edgeIdManager";
    public static final String GREMLIN_TINKERGRAPH_VERTEX_PROPERTY_ID_MANAGER = "gremlin.tinkercat.vertexPropertyIdManager";
    public static final String GREMLIN_TINKERGRAPH_DEFAULT_VERTEX_PROPERTY_CARDINALITY = "gremlin.tinkercat.defaultVertexPropertyCardinality";
    public static final String GREMLIN_TINKERGRAPH_GRAPH_LOCATION = "gremlin.tinkercat.graphLocation";
    public static final String GREMLIN_TINKERGRAPH_GRAPH_FORMAT = "gremlin.tinkercat.graphFormat";
    public static final String GREMLIN_TINKERGRAPH_ALLOW_NULL_PROPERTY_VALUES = "gremlin.tinkercat.allowNullPropertyValues";

    private final TinkerCatFeatures features = new TinkerCatFeatures();

    protected AtomicLong currentId = new AtomicLong(-1L);
    protected Map<Object, Vertex> vertices = new ConcurrentHashMap<>();
    protected Map<Object, Edge> edges = new ConcurrentHashMap<>();

    protected TinkerCatVariables variables = null;
    protected TinkerCatComputerView graphComputerView = null;
    ...
    /**
     * An empty private constructor that initializes {@link TinkerCat}.
     */
    private TinkerCat(final Configuration configuration) {
    ...
    /**
     * Open a new {@link TinkerCat} instance.
     * <p/>
     ...
     * {@code open()} method. This is an optional constructor method for TinkerCat. It is not enforced by the Gremlin
     * Test Suite.
     */
    public static TinkerCat open() {
        return open(EMPTY_CONFIGURATION);
    }

    /**
     * Open a new {@code TinkerCat} instance.
     * <p/>
     ...
     */
    public static TinkerCat open(final Configuration configuration) {
        return new TinkerCat(configuration);
    }
    ...
    public <C extends GraphComputer> C compute(final Class<C> graphComputerClass) {
        if (!graphComputerClass.equals(TinkerCatComputer.class))
            throw Graph.Exceptions.graphDoesNotSupportProvidedGraphComputer(graphComputerClass);
        return (C) new TinkerCatComputer(this);
    }

    @Override
    public GraphComputer compute() {
        return new TinkerCatComputer(this);
    }

    @Override
    public Variables variables() {
        if (null == this.variables)
            this.variables = new TinkerCatVariables();
    ...
    private <T extends Element> Iterator<T> createElementIterator(final Class<T> clazz, final Map<Object, T> elements,
        ...
        if (0 == ids.length) {
            iterator = new TinkerCatIterator<>(elements.values().iterator());
        } else {
            ...
            return new TinkerCatIterator<>(IteratorUtils.filter(IteratorUtils.map(idList, id -> {
    ...
    public class TinkerCatFeatures implements Features {

        private final TinkerCatGraphFeatures graphFeatures = new TinkerCatGraphFeatures();
        private final TinkerCatEdgeFeatures edgeFeatures = new TinkerCatEdgeFeatures();
        private final TinkerCatVertexFeatures vertexFeatures = new TinkerCatVertexFeatures();

        private TinkerCatFeatures() {
        }
    ...
    public class TinkerCatVertexFeatures implements Features.VertexFeatures {

        private final TinkerCatVertexPropertyFeatures vertexPropertyFeatures = new TinkerCatVertexPropertyFeatures();

        private TinkerCatVertexFeatures() {
        }
    ...
    public class TinkerCatEdgeFeatures implements Features.EdgeFeatures {

        private TinkerCatEdgeFeatures() {
        }
    ...
    public class TinkerCatGraphFeatures implements Features.GraphFeatures {

        private TinkerCatGraphFeatures() {
        }
    ...
    public class TinkerCatVertexPropertyFeatures implements Features.VertexPropertyFeatures {

        private TinkerCatVertexPropertyFeatures() {
        }
    ...
    /**
     * Construct an {@link TinkerCat.IdManager} from the TinkerCat {@code Configuration}.
     */
    private static IdManager<?> selectIdManager(final Configuration config, final String configKey, final Class<? extends Element> clazz) {
        ...
        try {
            ...
            try {
                ...
                throw new IllegalStateException(String.format("Could not configure TinkerCat %s id manager with %s", clazz.getSimpleName(), vertexIdManagerConfigValue));
            }
        }
    }
    ...
    public interface IdManager<T> {
        /**
         * Generate an identifier which should be unique to the {@link TinkerCat} instance.
         */
        T getNextId(final TinkerCat graph);
    ...
    public enum DefaultIdManager implements IdManager {
        ...
        LONG {
            @Override
            public Long getNextId(final TinkerCat graph) {
        ...
        INTEGER {
            @Override
            public Integer getNextId(final TinkerCat graph) {
        ...
        UUID {
            @Override
            public UUID getNextId(final TinkerCat graph) {
        ...
        /**
         * Manages identifiers of any type.  This represents the default way {@link TinkerCat} has always worked.
         ...
         * {@link TinkerCat}, it will generate {@link Long} values for identifiers.
         */
        ANY {
            @Override
            public Long getNextId(final TinkerCat graph) {
