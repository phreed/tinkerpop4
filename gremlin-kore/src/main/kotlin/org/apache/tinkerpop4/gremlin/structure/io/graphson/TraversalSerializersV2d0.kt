 ...
 */
internal class TraversalSerializersV2d0 private constructor() {
    /////////////////
    // SERIALIZERS //
    ////////////////
    ...
    internal class PJacksonSerializer : StdScalarSerializer<P?>(P::class.java) {
        ...
        fun serialize(p: P, jsonGenerator: JsonGenerator, serializerProvider: SerializerProvider?) {
            ...
            if (p is ConnectiveP) {
                ...
                if (p.getValue() is Collection) {
                    jsonGenerator.writeArrayFieldStart(GraphSONTokens.VALUE)
                    for (`object` in p.getValue()) {
    ...
    internal class PJacksonDeserializer : StdDeserializer<P?>(P::class.java) {
        @Override
        @Throws(IOException::class, JsonProcessingException::class)
            ...
            return if (predicate!!.equals(GraphSONTokens.AND) || predicate.equals(GraphSONTokens.OR)) {
                ...
                try {
                    if (value is Collection) {
                        ...
                        ) P.without(value as Collection?) else P::class.java.getMethod(
                            predicate,
                            Collection::class.java
                        ).invoke(null, value as Collection?) as P
                    } else {
                        try {
                            P::class.java.getMethod(predicate, Object::class.java).invoke(null, value) as P
                        } catch (e: NoSuchMethodException) {
                            P::class.java.getMethod(predicate, Array<Object>::class.java)
                                .invoke(null, arrayOf<Object?>(value) as Object?) as P
                        }
                    }
                } catch (e: Exception) {
    ...
    internal class TextPJacksonDeserializer : StdDeserializer<TextP?>(TextP::class.java) {
        @Override
        @Throws(IOException::class, JsonProcessingException::class)
