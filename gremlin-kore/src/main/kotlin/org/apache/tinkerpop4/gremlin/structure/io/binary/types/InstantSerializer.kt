 ...
 */
class InstantSerializer : SimpleTypeSerializer<Instant?>(DataType.INSTANT) {
    @Override
    @Throws(IOException::class)
    protected fun readValue(buffer: Buffer, context: GraphBinaryReader?): Instant {
        return Instant.ofEpochSecond(buffer.readLong(), buffer.readInt())
    }

    @Override
    @Throws(IOException::class)
    protected fun writeValue(value: Instant, buffer: Buffer, context: GraphBinaryWriter?) {
