 ...
 */
class SingleMessenger<M>(baseMessenger: Messenger<M>, message: M) : Messenger<M> {
    private val baseMessenger: Messenger<M>
    ...
    @Override
    fun receiveMessages(): Iterator<M> {
        return IteratorUtils.of(message)
    }

    @Override
    fun sendMessage(messageScope: MessageScope?, message: M) {
        baseMessenger.sendMessage(messageScope, message)
    }
}