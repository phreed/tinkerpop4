 ...
 */
class PathStep<S>(traversal: Traversal.Admin?) : MapStep<S, Path?>(traversal), TraversalParent, PathProcessor,
    ByModulating, FromToModulating {
    ...
    @Override
    fun clone(): PathStep<S> {
        val clone = super.clone() as PathStep<S>
    ...
    val requirements: Set<Any>
        get() = this.getSelfAndChildRequirements(TraverserRequirement.PATH)
    ...
    @Override
    protected fun processNextStart(): Traverser.Admin<Path> {
        val traverser: Traverser.Admin<S> = this.starts.next()
        val path: Path = traverser.path().subPath(fromLabel, toLabel)
        return if (traversalRing.isEmpty()) PathProcessor.processTraverserPathLabels(
            traverser.split(path, this),
            keepLabels
        ) else {
            traversalRing.reset()
            ...
            if (path.size() === byPath.size()) PathProcessor.processTraverserPathLabels(
                traverser.split(byPath, this),
                keepLabels
