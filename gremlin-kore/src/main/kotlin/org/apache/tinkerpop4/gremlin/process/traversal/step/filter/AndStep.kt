 ...
 */
class AndStep<S>(traversal: Traversal.Admin?, vararg traversals: Traversal<S, *>?) :
    ConnectiveStep<S>(traversal, traversals) {
    @Override
    protected fun filter(traverser: Traverser.Admin<S>?): Boolean {
        for (traversal in this.traversals) {
            if (!TraversalUtil.test(traverser, traversal)) return false
        }
        return true
    }
}