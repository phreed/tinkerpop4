...
class UUIDSerializer : SimpleTypeSerializer<UUID?>(DataType.UUID) {
    @Override
    protected fun readValue(buffer: Buffer, context: GraphBinaryReader?): UUID {
        return UUID(buffer.readLong(), buffer.readLong())
    }

    @Override
    protected fun writeValue(value: UUID, buffer: Buffer, context: GraphBinaryWriter?) {
