...
/**
 * GraphSON serializers for classes in `java.time.*`.
 */
internal class JavaTimeSerializersV1d0 private constructor() {
    /**
    ...
    internal abstract class AbstractJavaTimeJacksonDeserializer<T>(clazz: Class<T>?) : StdDeserializer<T>(clazz) {
        ...
        fun deserialize(jsonParser: JsonParser, deserializationContext: DeserializationContext?): T {
            if (!jsonParser.getText().equals(GraphSONTokens.VALUE)) throw IOException(
                String.format(
                    "Invalid format for %s - expecting '%s' with a text value in ISO-8061 format",
