 ...
 */
class AddEdgeStep<S> : ScalarMapStep<S, Edge?>, Mutating<EdgeAddedEvent?>, TraversalParent, Scoping, FromToModulating {
    ...
    constructor(traversal: Traversal.Admin?, edgeLabelTraversal: Traversal.Admin<S, String?>?) : super(traversal) {
    ...
    @Override
    protected fun map(traverser: Traverser.Admin<S>?): Edge {
        val edgeLabel: String = parameters.get(traverser, T.label) { Edge.DEFAULT_LABEL }.get(0)
        val theTo: Object = parameters.get(traverser, TO, traverser::get).get(0)
        ...
        val theFrom: Object = parameters.get(traverser, FROM, traverser::get).get(0)
        ...
        if (fromVertex is Attachable) fromVertex = (fromVertex as Attachable<Vertex?>)
            .attach(Attachable.Method.get(this.getTraversal().getGraph().orElse(EmptyGraph.instance())))
        val edge: Edge = fromVertex.addEdge(edgeLabel, toVertex, parameters.getKeyValues(traverser, TO, FROM, T.label))
    ...
    @Override
    fun clone(): AddEdgeStep<S> {
        val clone = super.clone() as AddEdgeStep<S>
