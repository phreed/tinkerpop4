 ...
 */
class DurationSerializer : SimpleTypeSerializer<Duration?>(DataType.DURATION) {
    @Override
    @Throws(IOException::class)
    protected fun readValue(buffer: Buffer, context: GraphBinaryReader?): Duration {
        return Duration.ofSeconds(buffer.readLong(), buffer.readInt())
    }

    @Override
    @Throws(IOException::class)
    protected fun writeValue(value: Duration, buffer: Buffer, context: GraphBinaryWriter?) {
