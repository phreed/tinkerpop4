...
class ListSerializer : SimpleTypeSerializer<List?>(DataType.LIST) {
    @Override
    @Throws(IOException::class)
    protected fun readValue(buffer: Buffer?, context: GraphBinaryReader?): List {
        // The collection is a List<>
        return collectionSerializer.readValue(buffer, context)
    }

    @Override
    @Throws(IOException::class)
    protected fun writeValue(value: List?, buffer: Buffer?, context: GraphBinaryWriter?) {
