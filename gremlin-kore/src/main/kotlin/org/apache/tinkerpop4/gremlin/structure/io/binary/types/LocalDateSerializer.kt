 ...
 */
class LocalDateSerializer : SimpleTypeSerializer<LocalDate?>(DataType.LOCALDATE) {
    @Override
    @Throws(IOException::class)
    protected fun readValue(buffer: Buffer, context: GraphBinaryReader?): LocalDate {
        return LocalDate.of(buffer.readInt(), buffer.readByte(), buffer.readByte())
    }

    @Override
    @Throws(IOException::class)
    protected fun writeValue(value: LocalDate, buffer: Buffer, context: GraphBinaryWriter?) {
