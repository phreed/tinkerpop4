 ...
 */
internal class GraphSONSerializersV2d0 private constructor() {
    ////////////////////////////// SERIALIZERS /////////////////////////////////
    ...
    internal class PathJacksonDeserializer : StdDeserializer<Path?>(Path::class.java) {
        ...
        fun deserialize(jsonParser: JsonParser, deserializationContext: DeserializationContext): Path {
            val n: JsonNode = jsonParser.readValueAsTree()
            val p: Path = MutablePath.make()
            val labels: ArrayNode = n.get(GraphSONTokens.LABELS) as ArrayNode
            val objects: ArrayNode = n.get(GraphSONTokens.OBJECTS) as ArrayNode
            for (i in 0 until objects.size()) {
                val po: JsonParser = objects.get(i).traverse()
                po.nextToken()
                val pl: JsonParser = labels.get(i).traverse()
                pl.nextToken()
                p.extend(
                    deserializationContext.readValue(po, Object::class.java),
                    deserializationContext.readValue(pl, setType)
                )
            }
            return p
        }
    ...
    internal class MetricsJacksonDeserializer : AbstractObjectDeserializer<Metrics?>(Metrics::class.java) {
        @Override
        fun createObject(metricsData: Map<String?, Object?>): Metrics {
            val m =
    ...
    internal class TraversalMetricsJacksonDeserializer : AbstractObjectDeserializer<TraversalMetrics?>(
        TraversalMetrics::class.java
    ) {
        @Override
        fun createObject(traversalMetricsData: Map<String?, Object?>): TraversalMetrics {
    ...
    internal class IntegerJacksonDeserializer protected constructor() : StdDeserializer<Integer?>(Integer::class.java) {
        @Override
        @Throws(IOException::class, JsonProcessingException::class)
