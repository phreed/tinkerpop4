...
class B_NL_O_S_SE_SL_Traverser<T> : B_O_S_SE_SL_Traverser<T> {
    ...
    fun <R> split(r: R, step: Step<T, R>?): Admin<R> {
        val clone = super.split(r, step) as B_NL_O_S_SE_SL_Traverser<R>
    ...
    @Override
    fun split(): Admin<T> {
        val clone = super.split() as B_NL_O_S_SE_SL_Traverser<T>
        clone.nestedLoops = Stack()
        for (lc in nestedLoops) clone.nestedLoops.push(lc.clone() as LabelledCounter)
        if (loopNames != null) {
            clone.loopNames = ReferenceMap(ReferenceMap.HARD, ReferenceMap.WEAK)
    ...
    override fun equals(o: Object): Boolean {
        ...
        if (o !is B_NL_O_S_SE_SL_Traverser<*>) return false
        if (!super.equals(o)) return false
        val that = o as B_NL_O_S_SE_SL_Traverser<*>
