 ...
 */
class ReferencePath : MutablePath, Attachable<Path?> {
    private constructor() {}
    ...
    protected constructor(path: Path) {
        path.forEach { `object`, labels ->
            if (`object` is ReferenceElement || `object` is ReferenceProperty || `object` is ReferencePath) this.objects.add(
                `object`
            ) else this.objects.add(ReferenceFactory.detach(`object`))
            labels.add(HashSet(labels))
        }
    }
