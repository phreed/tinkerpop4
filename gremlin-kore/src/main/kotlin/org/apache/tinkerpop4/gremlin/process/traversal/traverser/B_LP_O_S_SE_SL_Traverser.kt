 ...
 */
class B_LP_O_S_SE_SL_Traverser<T> : B_O_S_SE_SL_Traverser<T> {
    ...
    constructor(t: T, step: Step<T, *>, initialBulk: Long) : super(t, step, initialBulk) {
        ...
        val labels: Set<String> = step.getLabels()
        if (!labels.isEmpty()) path = path.extend(t, labels)
    }
    ...
    fun <R> split(r: R, step: Step<T, R>): Traverser.Admin<R> {
        val clone = super.split(r, step) as B_LP_O_S_SE_SL_Traverser<R>
    ...
    fun split(): Traverser.Admin<T> {
        val clone = super.split() as B_LP_O_S_SE_SL_Traverser<T>
        clone.path = clone.path.clone()
        return clone
    }
    ...
    fun addLabels(labels: Set<String?>) {
        if (!labels.isEmpty()) path =
            if (path.isEmpty() || null == this.t && path.head() == null || !this.t.equals(path.head())) path.extend(
                this.t,
                labels
            ) else path.extend(labels)
    }
    ...
    fun keepLabels(labels: Set<String?>) {
        ...
        for (stepLabels in path.labels()) {
            for (label in stepLabels) {
                if (!labels.contains(label)) retractLabels.add(label)
            }
        }
    ...
    protected fun equals(other: B_LP_O_S_SE_SL_Traverser<*>): Boolean {
        return super.equals(other) && other.path.equals(path)
    }

    @Override
    override fun equals(`object`: Object?): Boolean {
        return `object` is B_LP_O_S_SE_SL_Traverser<*> && this.equals(`object` as B_LP_O_S_SE_SL_Traverser<*>?)
    }
}