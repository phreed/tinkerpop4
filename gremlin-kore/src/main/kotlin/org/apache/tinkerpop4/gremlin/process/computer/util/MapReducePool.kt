 ...
 */
class MapReducePool(mapReduce: MapReduce, poolSize: Int) {
    private val pool: LinkedBlockingQueue<MapReduce<*, *, *, *, *>>

    init {
        pool = LinkedBlockingQueue(poolSize)
        while (pool.remainingCapacity() > 0) {
            pool.add(mapReduce.clone())
        }
    }
    ...
    fun offer(mapReduce: MapReduce<*, *, *, *, *>?) {
        try {
            pool.offer(mapReduce, TIMEOUT_MS, TimeUnit.MILLISECONDS)
        } catch (e: InterruptedException) {
            throw IllegalStateException(e.getMessage(), e)
        }
    }
