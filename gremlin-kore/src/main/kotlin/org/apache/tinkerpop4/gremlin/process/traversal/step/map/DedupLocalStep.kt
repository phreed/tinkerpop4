 ...
 */
class DedupLocalStep<E, S : Iterable<E>?>(traversal: Traversal.Admin?) : ScalarMapStep<S, Set<E>?>(traversal) {
    @Override
    protected fun map(traverser: Traverser.Admin<S>): Set<E> {
        val result: Set<E> = LinkedHashSet()
        for (item in traverser.get()) {
            result.add(item)
        }
        return result
    }
