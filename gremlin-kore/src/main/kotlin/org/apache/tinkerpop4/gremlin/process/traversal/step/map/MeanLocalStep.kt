 ...
 */
class MeanLocalStep<E : Number?, S : Iterable<E>?>(traversal: Traversal.Admin?) : ScalarMapStep<S, Number?>(traversal) {
    @Override
    protected fun map(traverser: Traverser.Admin<S>): Number {
        val iterator: Iterator<E> = traverser.get().iterator()
        if (iterator.hasNext()) {
            // forward the iterator to the first non-null or return null
            var result = untilNonNull(iterator)
            val counter = 1L
            while (iterator.hasNext()) {
                val n: Number? = iterator.next()
                if (n != null) {
                    result = NumberHelper.add(result, n)
                    counter++
                }
            }
