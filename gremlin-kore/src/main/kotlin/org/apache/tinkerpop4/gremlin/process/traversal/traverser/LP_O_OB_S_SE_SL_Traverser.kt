 ...
 */
class LP_O_OB_S_SE_SL_Traverser<T> : O_OB_S_SE_SL_Traverser<T> {
    ...
    fun <R> split(r: R, step: Step<T, R>): Traverser.Admin<R> {
        val clone = super.split(r, step) as LP_O_OB_S_SE_SL_Traverser<R>
    ...
    fun split(): Traverser.Admin<T> {
        val clone = super.split() as LP_O_OB_S_SE_SL_Traverser<T>
        clone.path = clone.path.clone()
        return clone
    }
    ...
    protected fun equals(other: LP_O_OB_S_SE_SL_Traverser<*>): Boolean {
        return super.equals(other) && other.path.equals(path)
    }

    @Override
    override fun equals(`object`: Object?): Boolean {
        return `object` is LP_O_OB_S_SE_SL_Traverser<*> && this.equals(`object` as LP_O_OB_S_SE_SL_Traverser<*>?)
    }
}